...
void TCPSender::fill_window(bool send_syn) {
    ...
    while((remain = win_size - (_next_seqno - _ackno)) != 0) {
        ...
        if (_stream.eof() && seg.length_in_sequence_space() < size) {            
            seg.header().fin = true;
            _fin_flag = true;
            send_segments(seg);
            return;
...
bool TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if(abs_ackno <= _ackno)
        return true;
    _ackno = abs_ackno;
    _left_timeout = _initial_retransmission_timeout;

    if(!_outstands.empty())
        while((abs_ackno  >= (unwrap(_outstands.begin()->second.header().seqno, _isn, _ackno) \
                        + _outstands.begin()->second.length_in_sequence_space()))\
            && (bytes_in_flight() > 0) && !_outstands.empty())
        {
            _flight_bytes -= _outstands.begin()->second.length_in_sequence_space();
...
//! \brief 空报文，并不关心是否被收到，没有数据，也没有序列号，也就被对等方的connect需要
//! \brief 一般，带SYN/FIN的不要用空报文，因为他们是要被对等方的receiver接收的
...
void TCPSender::send_segments(TCPSegment &seg) {
    ...
    if(! _timer_start)
        _timer_start = true;
    if(_outstands.begin() == _outstands.end())
        _left_timeout = _retransmit_timeout;
}

// void TCPSender::send_ack_segments(TCPSegment &seg) {
//     seg.header().seqno = wrap(_next_seqno, _isn);
//     seg.header().ack = true;
//     _outstands[_next_seqno] = seg;
//     _next_seqno += seg.length_in_sequence_space();
